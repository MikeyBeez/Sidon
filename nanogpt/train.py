...
while True:
    ...
    if iter_num % log_interval == 0 and master_process:
        ...
        sidon_str = f", sidon {last_sidon_loss:.4f}" if sidon_lambda > 0 else ""
        print(f"iter {iter_num}: loss {lossf:.4f}{sidon_str}, time {dt*1000:.2f}ms, mfu {running_mfu*100:.2f}%")
        lm_lossf = lossf - sidon_lambda * last_sidon_loss
        training_log.append({
            'iter': iter_num,
            'loss': lossf,
            'lm_loss': lm_lossf,
            'sidon_loss': last_sidon_loss,
            'lr': lr,
        })
    iter_num += 1
    local_iter_num += 1
...
if master_process and training_log:
